				     ...
				     u32 lane0_rate, u32 gen2_calib)
{
	...
	do {
		...
		for (loop = 0; loop <= 3; loop++) {
			if ((currbistresult[loop] == 1)
			    && (prevbistresult[loop] == 0)
			    && (prev2bistresult[loop] == 1)) {
				if (bistpasscount[loop] > 0)
					bistpassholes[loop] =
					    bistpassholes[loop] + 1;
			}
			if ((currbistresult[loop] == 1)
			    && (prevbistresult[loop] == 1)) {
				bistpasscount[loop] = bistpasscount[loop] + 1;
				bistpasscounttotal[loop] =
				    bistpasscounttotal[loop] + 1;
			}
			...
			if ((meancount[loop] == 0) && (bistpasscount[loop] >= 4)
				...
				meancount[loop] =
				    (itercount - 1) -
				    ((bistpasscount[loop] + 1) / 2);
				bistpasscountfinal[loop] =
				    bistpasscount[loop] + 1;
			}
	...
	if (gen2_calib != 1) {
		retval =
		    ((bistpasscount[3] < 8) << 3) | ((bistpasscount[2] <
						      8) << 2) |
		    ((bistpasscount[1] < 8) << 1) | (bistpasscount[0] < 8);

	}
	if (gen2_calib == 1) {
		retval =
		    (((bistpassholes[3] > 2) | (bistpasscounttotal[3] <
						18) | (bistpasscounttotal[3] >
						       37) |
		      ((bistpasscountfinal[3] + 1) <
		       bistpasscounttotal[3])) << 3) | (((bistpassholes[2] >
							  2) |
							 (bistpasscounttotal[2]
							  <
							  18) |
							 (bistpasscounttotal[2]
							  >
							  37) |
							 ((bistpasscountfinal[2]
							   + 1) <
							  bistpasscounttotal
							  [2])) << 2) |
		    (((bistpassholes[1] > 2) | (bistpasscounttotal[1] <
						18) | (bistpasscounttotal[1] >
						       37) |
		      ((bistpasscountfinal[1] + 1) <
		       bistpasscounttotal[1])) << 1) | (((bistpassholes[0] >
							  2) |
							 (bistpasscounttotal[0]
							  <
							  18) |
							 (bistpasscounttotal[0]
							  >
							  37) |
							 ((bistpasscountfinal[0]
							   + 1) <
							  bistpasscounttotal
							  [0])));
			   ...
			   u32 lane0_protocol, u32 lane0_rate)
{
	...
	if (lane0_protocol == 1) {
		if (lane0_protocol == 1)
			Xil_Out32(0xFD401914, 0xF2);
		if (lane1_protocol == 1)
			Xil_Out32(0xFD405914, 0xF2);
		if (lane2_protocol == 1)
			Xil_Out32(0xFD409914, 0xF2);
		if (lane3_protocol == 1)
			Xil_Out32(0xFD40D914, 0xF2);
		retval =
		    serdes_illcalib_pcie_gen1(0, lane3_protocol, 1,
					      lane2_protocol, 1, lane1_protocol,
					      1, lane0_protocol, 1, 1);
		if (lane0_protocol == 1)
			lockrange_gen2_bad[0] = (retval & 0x1);
		if (lane1_protocol == 1)
			lockrange_gen2_bad[1] = (retval & 0x2) >> 1;
		if (lane2_protocol == 1)
			lockrange_gen2_bad[2] = (retval & 0x4) >> 2;
		if (lane3_protocol == 1)
			lockrange_gen2_bad[3] = (retval & 0x8) >> 3;

		if (lane0_protocol == 1)
			Xil_Out32(0xFD401914, 0xF7);
		if (lane1_protocol == 1)
			Xil_Out32(0xFD405914, 0xF7);
		if (lane2_protocol == 1)
			Xil_Out32(0xFD409914, 0xF7);
		if (lane3_protocol == 1)
			Xil_Out32(0xFD40D914, 0xF7);
		retval =
		    serdes_illcalib_pcie_gen1(0, lane3_protocol, 0,
					      lane2_protocol, 0, lane1_protocol,
					      0, lane0_protocol, 0, 0);
		if (lane0_protocol == 1)
			lockrange_gen1_bad[0] = (retval & 0x1);
		if (lane1_protocol == 1)
			lockrange_gen1_bad[1] = (retval & 0x2) >> 1;
		if (lane2_protocol == 1)
			lockrange_gen1_bad[2] = (retval & 0x4) >> 2;
		if (lane3_protocol == 1)
			lockrange_gen1_bad[3] = (retval & 0x8) >> 3;
		...
		for (loop = 0; loop < 4; loop++) {
			if ((lockrange_gen2_bad[loop] == 1)) {
				...
				if (loop == 0)
					Xil_Out32(0xFD401914, 0x16);
				if (loop == 1)
					Xil_Out32(0xFD405914, 0x16);
				if (loop == 2)
					Xil_Out32(0xFD409914, 0x16);
				if (loop == 3)
					Xil_Out32(0xFD40D914, 0x16);
			}
		}

		serdes_illcalib_pcie_gen1(0, lane3_protocol, 0, lane2_protocol,
					  0, lane1_protocol, 0, lane0_protocol,
					  0, 0);
		if (lane0_rate == 1) {
...
static unsigned long psu_pll_init_data(void)
{
	...
	psu_mask_write(0xFF5E0030, 0x00717F00U, 0x00015800U);
	...
	psu_mask_write(0xFF5E0038, 0x8000FFFFU, 0x8000793EU);
...
static unsigned long psu_clock_init_data(void)
{
	...
	psu_mask_write(0xFD1A0074, 0x013F3F07U, 0x01011403U);
...
static unsigned long psu_ddr_init_data(void)
{
	...
	psu_mask_write(0xFD070064, 0x0FFF83FFU, 0x009280D3U);
...
static unsigned long psu_mio_init_data(void)
{
	...
	psu_mask_write(0xFF180204, 0xFFFFFFFFU, 0x52000000U);
	psu_mask_write(0xFF180208, 0xFFFFFFFFU, 0x00B03004U);
	psu_mask_write(0xFF18020C, 0x00003FFFU, 0x00000FC0U);
	psu_mask_write(0xFF180138, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF18013C, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF180140, 0x03FFFFFFU, 0x023FFEDEU);
	psu_mask_write(0xFF180144, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF180148, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF18014C, 0x03FFFFFFU, 0x01FFFDFFU);
	psu_mask_write(0xFF180154, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF180158, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF18015C, 0x03FFFFFFU, 0x01FFB9D5U);
	psu_mask_write(0xFF180160, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF180164, 0x03FFFFFFU, 0x03FFFFFFU);
	psu_mask_write(0xFF180168, 0x03FFFFFFU, 0x03F376EBU);
...
static unsigned long psu_serdes_init_data(void)
{
	...
	psu_mask_write(0xFD402860, 0x00000081U, 0x00000001U);
